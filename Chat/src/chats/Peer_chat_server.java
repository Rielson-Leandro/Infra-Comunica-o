	...
	public void run() {
		try {                                   
			...
			while (true) {
				...
				System.out.println("Eu tenho um novo pedido");
				...
				case List_Users:
				{
					System.out.println("Aqui eu estou escrevendo o tamanho da lista de usuário:"+chat_server.user_List.size());
					dos.reset();
				...
				case List_Groups:
				{
					System.out.println("Aqui eu estou escrevendo o tamanho da lista de usuário : "+chat_server.available_groups_list.size());
					dos.reset();
				...
				case group_chat_message:
					...
					System.out.println("solicitação de mensagem de grupo");
...
public class Peer_chat_server {
	...
	public void start() {
		...
		try {
			...
			System.out.println("Olá a partir do servidor");
			while (true) {


				Socket peer= sv.accept();
				System.out.println("Novos pares Chegou");
	...
	public void join(String data,User user) {
		...
		for (Available_group available_group : available_groups_list) 
		{
			if(available_group.name.equals(data))
			{
				...
				System.out.println("Usuario "+ user.username + "Adicionado a "+available_group.name );
			}
		}
...
class BroadcastThread extends Thread
{
	...
	public void run()
	{
		System.out.println("começar a transmissão "+ group.name+" "+"contendo "+group.group_useres.size());
		...
		for (User u : group.group_useres )
		{
			...
			{
				try 
				{	System.out.println("envio de" +chat_server.socket_map.get(u).user_identification.username );
